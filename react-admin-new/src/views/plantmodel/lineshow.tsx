            ...
            <Box sx={{width: 'calc(100vw - 450px)'}} >
                ...
                <Box sx={{ width: '100%', typography: 'body1' }}>
                    ...
                    <TabContext value={value}>
                        ...
                        <TabPanel value="2"><LineEquipInfo line_id = {id} /></TabPanel>
                        <TabPanel value="3">维护产线其他跟排产有关的信息</TabPanel>
                    </TabContext>
                </Box>
            </Box>
        </Show>
            ...
            <DialogContent dividers>
                ...
                > */}
                    ...
                    >
                        ...
                        />
                        <TextInput source = 'count' label = '员工人数' />
...
const LineEmpInfo: React.FC<{line_id: string | undefined}> = (props) => {
    ...
    const {data} = useGetList(
        't_lps_emp_skill',
        {
            sort: {field: 'skill_level', order: 'ASC'}
        }  
    );
    ...
    const handleSave = () => {
        ...
        if(skill.length !== 0){
            ...
            if(count === undefined) {
                notify('请填入该技能员工人数', {type: 'warning'});
                return;
            }
            //获得输入的数据插入数据
            create('t_lps_line_emp', {data: {skill_id, line_id, count: Number(count), description: desc}}, {
                onSuccess: () => {
                    setDesc('');
                    setCount('');
                    refresh();
        ...
        <Stack direction= 'row'>
            <Paper sx = {{width: '20%'}} elevation={1}>
                <ul style = {{marginBottom: '0px', marginTop: '0px', padding: '0 8px 0 8px'}}>
                    <li style = {{backgroundColor: '#cdcdcd', listStyle: 'none', textAlign: 'center', fontWeight: 'bold'}}>员工技能列表</li>
                </ul>
                <ul style = {{listStyle: 'none', padding: '8px', margin: '0 0 0 0'}}>
                    {
                        skillList && skillList.map((item, idx) => {
                            ...
                            return (
                                <SkillLi style = {bgcolor} key = {idx} onClick={()=>handleClick(idx)}>{ item['description']} </SkillLi>
                            )
                        })
                    }
                </ul>
            ...
            <Paper sx = {{width: '60%', marginLeft: '4px', marginRight: '4px'}} elevation={1}>
                <List resource = 't_lps_line_emp'
                    exporter = {false} hasCreate = {false}
                    filter={{line_id: props.line_id}}
                    empty = {<Empty />}
                >
                    <Datagrid>
                        <ReferenceField source = 'skill_id' label = '技能级别' reference="t_lps_emp_skill" />
                        <TextField source = 'count' label = '人数'  />
                        <TextField source = 'description' label = '说明' />
                        <ChangeButton />
                    </Datagrid>
                </List>
            ...
            <Paper sx = {{width: '20%', position: 'relative'}} elevation={1} >
                <div style = {{position: 'absolute', bottom: '20px'}}>
                    ...
                    />
                    <MuiTextField sx = {{ml: '8px'}} label = '输入该技能人数' variant= 'standard'
                        value={count}
